...
struct Edge {
    ...
    pair<double, double>intermediate_point;//中转点
    ...
    Edge(int from = -1, int to = -1, double distance = 0.0, EdgeType type = EdgeType::ROAD, int riskLevel = 0, double congestion = 0.0,double intermediate_point_x=0.0,double intermediate_point_y = 0.0) :
        from(from), to(to), distance(distance), type(type), riskLevel(riskLevel), congestion(congestion){
        intermediate_point.first = intermediate_point_x;
        intermediate_point.second = intermediate_point_y;
    }
