...
public:
    struct ExitID_Length_Path{
        ...
        ExitID_Length_Path(
            ...
            : exitID(_exitid), length(_length)
            , path(std::move(_path)), edges(std::move(_edges))
        {}
    };
    /////////////////////////////
    // 1.编辑操作
    ...
    // 添加一条边
    // 在具体使用的过程中，可以如此编写：
    // 0 |Edge edges(...); // 先设置好边的属性
    // 1 |graph.addEdge(edges);
    // @param edges 添加的边。
    ...
    ///////////////////////////////
    // 3.核心算法
    ExitID_Length_Path findShortestPath(int startId, int endId, bool emergencyMode)const;// 使用Dijkstra算法查找最短路径
    // 多出口疏散方案
